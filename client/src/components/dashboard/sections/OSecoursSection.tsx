...
import { fetchServiceRequests, createServiceRequest, updateServiceRequestStatus } from '@/services/emergencyService';
...
const OSecoursSection = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [balancesRes, transactionsRes, requests] = await Promise.all([
        TokenService.getTokenBalances(),
        TokenService.getTransactions(user.id),
        fetchServiceRequests(),
      ]);

      if (balancesRes.success) setTokenBalances(balancesRes.data || []);
      ...
      toast({
        title: t('error'),
        description: t('error_fetch_data'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handlePurchase = useCallback(async () => {
    if (!selectedToken || !purchaseAmount || !phoneNumber) {
      toast({
        title: t('error'),
        description: t('error_select_token_amount_phone'),
        variant: 'destructive',
      });
      return;
    }
    ...
    if (isNaN(amount) || amount < MIN_PURCHASE_PER_SERVICE || amount > MAX_MONTHLY_PURCHASE_PER_SERVICE) {
      toast({
        title: t('error'),
        description: t('error_invalid_amount', { min: MIN_PURCHASE_PER_SERVICE, max: MAX_MONTHLY_PURCHASE_PER_SERVICE }),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const response = await TokenService.purchaseTokens(selectedToken, amount);
      ...
      toast({
        ...
        description: t('purchase_success_message', { amount }),
      });
  ...
  const requestService = useCallback(
    async (serviceId: string) => {
      if (!isEligible || !user?.id) {
        toast({
          title: t('error'),
          description: t('error_unauthorized'),
          variant: 'destructive',
        });
        return;
      }
      ...
      if (!service) {
        toast({
          title: t('error'),
          description: t('error_service_not_found'),
          variant: 'destructive',
        });
        return;
      }
      ...
      if (balance < 1) {
        toast({
          title: t('error'),
          description: t('error_insufficient_balance'),
          variant: 'destructive',
        });
      ...
      try {
        ...
        if (request) {
          ...
          toast({
            title: t('service_request_success'),
            description: t('service_request_success_message', { service: service.name }),
          });
        }
      } catch (err) {
        ...
        toast({
          title: t('error'),
          description: t('error_service_request'),
          variant: 'destructive',
        });
      }
    },
