...
DO $$
DECLARE
    target_user_id UUID := 'c7bc3f81-a80c-4bc9-bceb-e84cb3166431'; -- 🔧 VOTRE ID
    user_name TEXT;
    user_phone TEXT;
    user_email TEXT;
    affiliate_code TEXT;
BEGIN
    -- 1) Vérifier que l'utilisateur existe et récupérer l'email depuis auth.users
    SELECT 
        p.full_name, 
        p.phone, 
        au.email
    INTO user_name, user_phone, user_email
    FROM profiles p
    ...
    RAISE NOTICE '📧 Email (auth.users): %', COALESCE(user_email, 'N/A');
...
-- 8) Vérification finale avec email depuis auth.users
DO $$
DECLARE
    target_user_id UUID := 'c7bc3f81-a80c-4bc9-bceb-e84cb3166431'; -- 🔧 MÊME ID
...
BEGIN
    ...
    IF verification_result.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Email (auth.users): %', verification_result.email;
...
-- 9) Bonus: Vérifier les données auth.users vs profiles
SELECT 
...
FROM auth.users au 
WHERE au.id = 'c7bc3f81-a80c-4bc9-bceb-e84cb3166431'
...
SELECT 
    'profiles' as source,
    p.id,
    p.full_name as email_equivalent,
    p.created_at as profile_created_at
FROM profiles p 
WHERE p.id = 'c7bc3f81-a80c-4bc9-bceb-e84cb3166431';
