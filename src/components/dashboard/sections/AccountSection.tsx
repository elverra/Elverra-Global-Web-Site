            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                {userCards.map((card, index) => {
                  ...
                  // Si on n'a toujours pas de tier, on utilise 'unknown'
                  if (!cardTier) {
                  ...
                  const cardLabels: Record<string, { label: string; color: string }> = {
                    'child': { label: 'Carte Enfant', color: 'bg-pink-500' },
                    'kiddies': { label: 'Carte Enfant', color: 'bg-pink-500' },
                    'premium': { label: 'Carte Adulte Premium', color: 'bg-blue-500' },
                    'elite': { label: 'Carte Adulte Elite', color: 'bg-purple-600' },
                    'essential': { label: 'Carte Adulte Essentiel', color: 'bg-gray-500' },
                    'unknown': { label: 'Type de carte inconnu', color: 'bg-gray-400' }
                  };

                  const cardLabel = cardLabels[normalizedTier] || cardLabels['unknown'];
                ...
                <MemberDigitalCard
                  ...
                  userID={user?.id || ''}
                  expiryDate={(() => {
                    // Si on a une date d'expiration, on l'utilise
                    if (membership?.expiry_date) {
                      return new Date(membership.expiry_date).toISOString();
                    }
                    // Sinon, on calcule 1 an après la date de création de la carte
                    else if (currentCard?.created_at) {
                      const createdAt = new Date(currentCard.created_at);
                      const expiryDate = new Date(createdAt);
                      expiryDate.setFullYear(expiryDate.getFullYear() + 1);
                      return expiryDate.toISOString();
                    }
                    // Si aucune date n'est disponible, on retourne undefined
                    return undefined;
                  })()}
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="space-y-3">
                    ...
                    {userCards.length > 0 ? (
                      userCards.map((card, index) => {
                        // Déterminer le type de carte de manière fiable
                        const qrData = parseQRData(card.qr_data);
                        let cardType = '';
                        ...
                        const cardLabels: Record<string, { label: string; color: string }> = {
                          ...
                          'kiddies': { label: 'Carte Enfant', color: 'pink' },
                          'premium': { label: 'Carte Adulte Premium', color: 'blue' },
                          'elite': { label: 'Carte Adulte Elite', color: 'purple' },
                          'essential': { label: 'Carte Adulte Essentiel', color: 'gray' },
                          'unknown': { label: 'Type de carte inconnu', color: 'gray' }
                        };
