...
const AffiliateSection = () => {
  ...
  const checkAffiliateStatus = async () => {
    ...
    try {
      ...
      if (affiliate) {
        ...
        if (!affiliate.approved) {
          toast({
            ...
            description:
              "Votre demande d'affiliation est en cours de traitement.",
            variant: "default",
          });
        } else {
          toast({
      ...
      toast({
        title: "Erreur",
        description:
          "Impossible de charger les données d'affiliation. Veuillez réessayer plus tard.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleAnswerSubmit = async () => {
    ...
    const correctAnswers = currentLesson.questions.filter(
      (q, i) => userAnswers[`${onboardingStep}-${i}`] === q.correctAnswer
    ).length;
    ...
    if (isPassingScore) {
      const completedLessons = [
        ...new Set([...(userProgress.completed_lessons || []), onboardingStep]),
      ];
      ...
      try {
        ...
        toast({
          title: "Erreur",
          description:
            "Une erreur est survenue lors de la mise à jour de votre progression.",
          variant: "destructive",
        });
      }
    } else {
      ...
      toast({
        title: "Try Again",
        description: `You got ${correctAnswers} out of ${totalQuestions} correct. You need at least ${Math.ceil(
          totalQuestions * 0.7
  ...
  const enrollInAffiliateProgram = async () => {
    ...
    if (userProgress && !userProgress.onboarding_complete) {
      ...
      toast({
        ...
        description:
          "Veuvez d'abord terminer la formation avant de rejoindre le programme d'affiliation.",
        variant: "default",
      });
      setShowOnboarding(true);
      return;
    ...
    if (userProgress && !userProgress.quiz_passed) {
      ...
      toast({
        title: "Quiz requis",
        description:
          "Veuvez d'abord réussir le quiz pour rejoindre le programme d'affiliation.",
        variant: "destructive",
      });
      setShowOnboarding(true);
      setShowQuiz(true);
  ...
  const loadReferrals = async () => {
    ...
    try {
      ...
      const { data: referredUsers, error: refError } = await supabase
        ...
        .or(
          `referrer_affiliate_code.eq.${currentUserData.affiliate_code},referred_by.eq.${user.id}`
        );
      ...
      if (referredUsers && referredUsers.length > 0) {
        ...
        if (upsertError) {
          console.error(
            "Erreur lors de la création des commissions:",
            upsertError
          );
        }
      }
      ...
      if (error) throw error;
      console.log("Commissions récupérées:", commissions);
      ...
      if (affiliateData.id) {
        setAffiliateData((prev) => ({
          ...
          totalEarnings: formattedReferrals.reduce(
  ...
  const hasAffiliateData =
    affiliateData && (affiliateData.id || affiliateData.user_id);
        ...
        <CardContent className="space-y-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            ...
            <ul className="text-sm space-y-2 text-gray-700">
              ...
              <li>
                • Soyez payé pour chaque renouvellement et paiement de carte
              </li>
        ...
        <CardContent className="space-y-4">
          <div className="bg-yellow-50 p-4 rounded-lg">
            <p className="text-yellow-800">
              Votre demande d'adhésion au programme d'affiliation est en cours
              d'examen. Vous recevrez une notification par e-mail une fois
              approuvé.
            </p>
  ...
  if (!isApproved) {
    return (
      <Card>
        ...
        <CardContent>
          <p>
            Votre compte d'affiliation est en cours d'activation. Veuillez
            patienter.
          </p>
          {affiliateData.affiliate_code ? (
      ...
      <Dialog open={showOnboarding} onOpenChange={setShowOnboarding}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <div className="flex justify-between items-center">
              <DialogTitle>
                Formation des Affiliés - Étape {onboardingStep + 1} sur{" "}
                {lessons.length}
              </DialogTitle>
              <Button
                variant="ghost"
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between pt-4">
                ...
                <Button
                  ...
                  disabled={lessons[onboardingStep].questions.some(
                    (_, i) => !userAnswers[`${onboardingStep}-${i}`]
                  )}
                >
                  {onboardingStep === lessons.length - 1
                    ? "Terminer"
                    : "Suivant"}
                  <ChevronRight className="h-4 w-4 ml-2" />
                </Button>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button
              className="w-full"
              onClick={() =>
                alert("La fonctionnalité de retrait sera bientôt disponible !")
              }
            >
        ...
        <CardContent>
          {referrals.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              <p>
                Vous n'avez pas encore de parrainages. Partagez votre lien pour
                commencer à gagner !
              </p>
            </div>
          )}
