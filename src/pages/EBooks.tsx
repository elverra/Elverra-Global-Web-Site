...
import { BookOpen, Download, Star, Clock, Users, Eye, X, Loader2 } from 'lucide-react';
...
// Alternative: Désactivez le worker si nécessaire pour le débogage
...
const EBooks = () => {
  ...
  const fetchEbooks = useCallback(async () => {
    try {
      ...
      const totalDownloads = ebooksData.reduce((sum: number, book: Ebook) => sum + book.downloads, 0);
      const averageRating = ebooksData.length > 0
        ? ebooksData.reduce((sum: number, book: Ebook) => sum + book.rating, 0) / ebooksData.length
        : 0;
  ...
  const filteredBooks = useMemo(() => ebooks.filter(book => {
    const matchesCategory = selectedCategory === "All Categories" || book.category === selectedCategory;
    const matchesSearch = book.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      book.author.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (book.tags ? book.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())) : false);
    return matchesCategory && matchesSearch;
  }), [ebooks, selectedCategory, searchTerm]);

  const handleDownload = useCallback(async (book: Ebook) => {
    try {
  ...
  const handleReadNow = useCallback((book: Ebook) => {
    ...
    <MembershipGuard requiredFeature="ebooks">
      <Layout>
              ...
              <DialogContent className="max-w-6xl h-[80vh] p-0 flex flex-col">
                <DialogHeader className="px-6 pt-6 pb-2 border-b">
                  <div className="flex justify-between items-center">
                    <div>
                      <DialogTitle>{selectedBook?.title || 'E-Book Viewer'}</DialogTitle>
                      <DialogDescription>
                        {selectedBook?.author || 'Loading book...'}
                      </DialogDescription>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  {selectedBook ? (
                    <div className="flex-1 overflow-auto bg-gray-100">
                      <div className="flex justify-center p-4 h-full">
                        <iframe
                          ...
                          title={`Aperçu de ${selectedBook.title}`}
                          allowFullScreen
                        >
                          <p>Votre navigateur ne supporte pas les iframes. <a href={selectedBook.file_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Téléchargez le PDF</a> pour le consulter.</p>
                        </iframe>
                      </div>
                    </div>
                  ) : (
                    <div className="flex-1 flex items-center justify-center bg-gray-100">
                      <div className="text-center p-8">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-gray-900">Aucun livre sélectionné</h3>
                        <p className="mt-1 text-sm text-gray-500">Sélectionnez un livre dans la liste pour le consulter.</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        {selectedBook ? (
          ...
          <div className="flex-1 flex items-center justify-center bg-gray-100">
            <p className="text-gray-500">Sélectionnez un livre pour le prévisualiser</p>
          </div>
        )}
      </Layout>
    </MembershipGuard >
...
const EbookCard = ({ book, onReadNow, onDownload }: { book: Ebook; onReadNow: (book: Ebook) => void; onDownload: (id: string, url: string) => void }) => {
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="p-4">
        <h3 className="font-semibold text-gray-900 mb-1 line-clamp-2 h-12">{book.title}</h3>
