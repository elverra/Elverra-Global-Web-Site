...
const Career = () => {
  ...
  const fetchJobs = async () => {
    try {
      ...
      let query = supabase.from('elverra_jobs').select('*').eq('is_active', true);
  ...
  const incrementJobViews = async (jobId: string) => {
    try {
      ...
      await supabase.from('elverra_jobs').update({ views: newViews }).eq('id', jobId);
    } catch (error) {
  ...
  const formatSalary = (min?: number, max?: number, currency: string = 'FCFA') => {
    ...
    if (min) return `Starting from ${min.toLocaleString()} ${currency}`;
          ...
          <div className="container mx-auto px-4">
            <div className="text-center">
              <h1 className="text-4xl md:text-5xl font-bold mb-4">
                Careers at Elverra Global
              </h1>
              <p className="text-xl mb-8 max-w-3xl mx-auto">
                Join our dynamic team and contribute to transforming Africa’s financial future
              </p>
              ...
              {jobs.map((job) => (
                <Card
                  ...
                  className={`hover:shadow-lg transition-shadow ${
                    job.is_featured ? 'border-purple-200 bg-purple-50' : ''
                  }`}
                >
                  <CardContent className="p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                      ...
                      <div className="mt-4 lg:mt-0 lg:ml-6">
                        ...
                        >
                          <Link to={`/career/${job.id}`}>View job</Link>
                        </Button>
                      </div>
                    </div>
                  </CardContent>
