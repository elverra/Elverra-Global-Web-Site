...
export default function DiscountDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (arr.length === 0 && normalized.logo_url)
          arr.push(normalized.logo_url);
        setImages(arr);
      } catch (e) {
      ...
      try {
        ...
        if (merchant?.sector_id) {
          const { data: recs, error: recErr } = await supabase
            .from("discount_merchants")
            .select(
              "id,name,sector_id,discount_percentage,location,description,website,logo_url,cover_image_url,is_featured,is_active,created_at,rating"
            )
            .eq("is_active", true)
  ...
  const isAdvantagesUrl =
    merchant.advantages && /^(http|https):\/\//i.test(merchant.advantages);

  const coverSrc = images[slide];

  const nextSlide = () =>
    setSlide((s) => (images.length ? (s + 1) % images.length : s));
  const prevSlide = () =>
    setSlide((s) =>
      images.length ? (s - 1 + images.length) % images.length : s
    );
    ...
    try {
      ...
      if (myReview.id) {
        ...
        setReviews((prev) =>
          prev.map((r) =>
            r.id === myReview.id
              ? { ...r, rating: myReview.rating, comment: myReview.comment }
              : r
          )
        );
  ...
  const deleteReview = async (rid: string) => {
    ...
    try {
      ...
      if (myReview?.id === rid)
        setMyReview({ id: undefined, rating: 5, comment: "" });
  ...
  return (
    <Layout>
      <div className="container mx-auto p-4 md:p-6">
                  ...
                  <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
                    {images.map((_, i) => (
                      <span
                        ...
                        style={{
                          backgroundColor:
                            i === slide ? "white" : "rgba(255,255,255,0.5)",
                        }}
                      />
                    ))}
                  </div>
            ...
            <Card>
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  {merchant.location || "Location not provided"}
                </div>
        ...
        {/* Gallery */}
        {Array.isArray(merchant.gallery_urls) &&
          merchant.gallery_urls.length > 0 && (
            <div className="mb-8">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {merchant.gallery_urls.map((url, idx) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      const pos = images.findIndex((u) => u === url);
                      if (pos >= 0) setSlide(pos);
                    }}
                    className="aspect-square overflow-hidden rounded bg-gray-100 focus:outline-none"
                  >
                    <img
                      src={url}
                      alt={`gallery-${idx}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            <CardContent className="p-5 space-y-3">
              ...
              {!merchant.advantages && (
                <p className="text-gray-600">No advantages provided.</p>
              )}
              {merchant.advantages &&
                (isAdvantagesUrl ? (
                  <Button
        ...
        {(merchant.location_map_url || merchant.location) && (
          <div className="mt-8">
            ...
            <div className="mt-2 flex items-center gap-2 text-sm text-gray-600">
              <MapPin className="h-4 w-4" />{" "}
              {merchant.location || "Address not provided"}
              <Button
                variant="outline"
                size="sm"
                className="ml-auto"
        ...
        <div className="mt-10">
          <h3 className="font-semibold text-lg mb-3">
            User reviews {avgRating ? `(average ${avgRating.toFixed(1)})` : ""}
          </h3>
          {user ? (
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  ...
                  >
                    <Star
                      className="h-5 w-5"
                      fill={
                        (myReview?.rating || 0) >= n ? "currentColor" : "none"
                      }
                    />
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {recommendations.map((rec) => (
                <Card
                  ...
                  onClick={() => navigate(`/discounts/${rec.id}`)}
                >
                  <div className="aspect-video bg-gray-100">
                    <img
                      src={rec.cover_image_url || rec.logo_url || ""}
                      alt={rec.name}
                      className="w-full h-full object-cover"
                    />
                  </div>
